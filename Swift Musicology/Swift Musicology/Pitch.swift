...
public struct Pitch: Codable, Equatable, Comparable {
    /// Key of the pitch like C, D, A, B with accidentals.
    public var key: Key
    
    /// Octave of the pitch. In theory this must be zero or a positive integer. But `Pitch`
    /// does not limit octave and calculates every possible octave including the negative ones.
    ...
    /// Calculates and returns the frequency of note on octave based on its location of
    /// piano keys. Bases A4 note of 440Hz frequency standard.
...
extension Pitch: RawRepresentable {
    ...
    /// Returns midi note number. In theory, this must be in range [0 - 127], but it does
    /// not limits the midi note value.
...
/// - Returns: `Intreval` between two pitches. You can get the semitones from interval as well.
...
/// Calculates the `Pitch` above semitones.
///
/// - Parameters:
///   - note: The pitch that is being added semitones.
///   - semitone: semitones above.
/// - Returns: Returns `Pitch` above semitones.
public func + (pitch: Pitch, semitone: Int) -> Pitch {
    return Pitch(midiNote: pitch.rawValue + semitone)
}

/// Calculates the `Pitch` below semitones.
///
/// - Parameters:
///   - note: The pitch that is being calculated.
///   - semitone: semitones below.
/// - Returns: Returns `Pitch` below semitones.
public func - (pitch: Pitch, semitone: Int) -> Pitch {
    return Pitch(midiNote: pitch.rawValue - semitone)
}
